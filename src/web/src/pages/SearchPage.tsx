...
import { gradientFromSlug } from './GenrePage';
...
function GenresBrowse() {
    ...
    const sorted = [...q.data].sort((a, b) => (b.trackCount ?? 0) - (a.trackCount ?? 0));

    return (
        <section className="space-y-4 pt-4">
            <div className="flex items-baseline justify-between border-b border-border pb-4">
                <h2 className="text-2xl font-bold tracking-tight text-fg">Жанры</h2>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                {sorted.map((g) => (
                    <Link
                        key={g.id}
                        to={`/genres/${encodeURIComponent(g.slug)}`}
                        className={cn(
                            "relative aspect-[4/3] rounded-xl overflow-hidden p-4 flex flex-col justify-between",
                            "bg-gradient-to-br text-white shadow-sm hover:shadow-xl transition-all",
                            "hover:scale-[1.02] active:scale-[0.99]",
                            gradientFromSlug(g.slug)
                        )}
                    >
                        <span className="font-black text-lg md:text-xl tracking-tight drop-shadow-sm leading-tight">
                            {g.displayName}
                        </span>
                        {typeof g.trackCount === 'number' && (
                            <span className="text-[11px] font-bold uppercase tracking-widest opacity-90">
                                {g.trackCount} {pluralTracksShort(g.trackCount)}
                            </span>
                        )}
...
function pluralTracksShort(n: number): string {
    const last2 = n % 100;
    const last1 = n % 10;
