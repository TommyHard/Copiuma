...
function matchAndPatch(
    ...
    if (Array.isArray(data)) {
        ...
        const next = data.map((item) => {
            ...
            // FavoriteItem[] — состав списка
            // Если стало unliked — убираем; если liked — кэш всё равно
            // обновится при invalidate (новый объект с правильным likedAt)
            if (obj.id === trackId && !('isLikedByMe' in obj) && 'likedAt' in obj && !nextValue) {
                changed = true;
                return null;
            }

            // Обычный TrackListItem
            if (obj.id === trackId && obj.isLikedByMe !== nextValue) {
                changed = true;
                return { ...obj, isLikedByMe: nextValue };
            }

            // PlaylistTrack — там trackId
